...
def fetch_merriweather():
    ...
    font_dest = Path("vendor/fonts/Merriweather.ttf")
    if font_dest.exists():
        return font_dest
    ...
    with httpx.Client(follow_redirects=True) as client:
        ...
        if r.status_code == 200:
            ...
            raise Exception(f"Failed to download font: HTTP {r.status_code}")
    return font_dest
...
def draw_custom_image(header: str, subheader: str) -> Image.Image:
    """
    Create an image with custom header and subheader text
    """
    merriweather_font_path = fetch_merriweather()
    image = create_canvas()
    draw = ImageDraw.Draw(image)
    ...
    draw.text((header_x, TOP_MARGIN), header, fill="black", font=header_font)
    ...
    # Position subheader below header with spacing
    subheader_y = TOP_MARGIN + header_dims.height + 20
